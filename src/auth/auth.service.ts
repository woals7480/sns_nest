...
export class AuthService {
     ...
     * 2) 로그인 할때는 Basic토큰과 함께 요청을 보낸다.
     * Basic토큰은 '이메일:비밀번호'를 Base64로 인코딩한 형태이다.
     * 예) {authrization: 'Basic {token}'}
     * 3) 아무나 접근할 수 없는 (private route)를 접근 할 때는
     * accessToken을 Header에 추가해서 요청과 함께 보낸다.
     * 예) {authrization: 'Bearer {token}'}
     * 4) 토큰과 요청을 함께 받은 서버는 토큰 검증을 통해 현재 요청을 보낸
     * 사용자가 누구인지 알 수 있다.
     * 예를 들어서 현재 로그인한 사용자가 작성한 포스트만 가져오려면
     * 토콘의 sub 값에 입력돼있는 사용자의 포스트만 따로 필터링 할 수 있다.
    ...
    decodeBasicToekn(base64String: string) {
    ...
    /**
     * 1) registerWithEmail
     *  - email, password, nickname 입력받고 사용자 생성
     *  - 생성이 완료되면 accessToken과 refreshToken을 반환 -> 회원가입 후 다시 로그인하기 방지
     *
     * 2) loginWithEmail
     *  - email, password 입력받고 사용자 검증
     *  - 검증이 완료되면 accessToken과 refreshToken을 반환
     *
     * 3) loginUser
     *  - (1)과 (2)에서 필요한 accessToken과 refreshToken 반환하는 로직
     *
     * 4) signToken
     *  - (3)에서 필요한 accessToken과 refreshToken을 sign하는 로직
     * 
     * 5) authenticateWithEmailAndPassword
     *  - (2)에서 로그인을 진핼할때 필요한 기본적인 검증
    ...
    async registerWithEamil(user: RegisterUserDto) {
